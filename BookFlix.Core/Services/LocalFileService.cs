...
namespace BookFlix.Core.Services
{
    public class LocalFileService : IFileService
    {
        ...
        public async Task<UploadeBookValidationResult> UploadFileAsync(int bookId, IFormFile file)
        {
            var result = new UploadeBookValidationResult();
            ...
            var bookFileLocation = fileResult.FileLocation;
            using var transaction = await _bookRepository.BeginTransactionAsync();
            try
            {
                DeleteBookFile(bookFileLocation, result);
                if (!result.IsValid)
                {
                ...
                bookFileLocation = SetAutoGeneratedFileName();
                await SaveFileToDirectory(file, bookFileLocation);

                var fileNameAsFileLocation = Path.GetFileName(bookFileLocation);
                await _bookRepository.UpdateFileLocationAsync(bookId, fileNameAsFileLocation);
                await transaction.CommitAsync();

                result.FileLocation = fileNameAsFileLocation;
                return result;
            }
